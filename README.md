...
- 回读与 staged 补丁全部只写 SRAM/DDR **运行内存**，不碰设备持久存储——**破解阶段没有变砖
  风险**：写错、失败、异常，断电重启即完全恢复，重进下载模式再来一遍即可
...
3. **变砖风险（分阶段看）**：破解/回读阶段的补丁全部临时加载在内存（SRAM/DDR），断电重启
   即清空——**这一阶段没有变砖风险**，写错重来即可；只有走到**解锁/持久化阶段**（oem unlock
   回写 NV、写分区等持久操作）才可能出现难以自动恢复的问题，解锁前务必完成分区备份
   （unlock_chain.py 内置 upload_storage 分区备份）。
